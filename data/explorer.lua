...
local function find_closest_walkable_point(start, goal, max_distance)
    --console.print("Procurando ponto caminhável mais próximo do alvo.")
...
local function find_partial_path(start, goal)
    --console.print("Iniciando busca de caminho parcial de " .. vec3_to_string(start) .. " para " .. vec3_to_string(goal))
    local path = {start}
    local current = start
    local max_steps = 100
    ...
    for i = 1, max_steps do
        ...
        if not next_point then
            --console.print("Nenhum ponto caminhável encontrado em direção ao alvo.")
            break
        end
        
        --console.print("Próximo ponto encontrado: " .. vec3_to_string(next_point))
    ...
    --console.print("Caminho parcial finalizado com " .. #path .. " pontos")
    return path
end
